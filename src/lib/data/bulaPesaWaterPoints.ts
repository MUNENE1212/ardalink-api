...
// AUTO-GENERATED from OpenStreetMap (Overpass API) on 2026-05-20.
  ...
  export type WaterPointType =
    | "borehole" | "well" | "water_tower"
    | "drinking_water_point" | "livestock_watering_point"
    | "spring" | "water_point";
  ...
  export function formatWaterPointsBlock(): string {
    ...
    const anchorOk = new Set(["settlement", "school", "worship", "health", "market", "civic", "fuel"]);
    const candidates = BULA_PESA_LANDMARKS.filter((l) => anchorOk.has(l.category));
    ...
    const byQuad: Record<string, WaterPoint[]> = { NW: [], NE: [], SW: [], SE: [] };
    ...
    const fmtPoint = (p: WaterPoint): string => {
      ...
      if (anchor && anchor.d <= MAX_ANCHOR_KM) {
        ...
        facts.push(`locator: ${p.village} village (no nearby landmark within ${MAX_ANCHOR_KM} km)`);
      } else {
        facts.push(`locator: ${p.area} (no nearby landmark within ${MAX_ANCHOR_KM} km, no village tag)`);
      }
      const nameTag = hasMetadataBackedName(p) ? p.name : `${p.name} (name unconfirmed in OSM)`;
    ...
    const backedCount = BULA_PESA_WATER_POINTS.filter(hasMetadataBackedName).length;
