...
const TENANT_ATTESTATION_SECRET = process.env.TENANT_ATTESTATION_SECRET ?? '';
...
export function verifyJwt(token: string, secret: string): TenantClaims {
  ...
  const claims = JSON.parse(Buffer.from(payloadB64, 'base64url').toString('utf8')) as TenantClaims;
  if (!claims.tenant_id) {
...
export function attestTenant(tenantId: string): string {
  ...
  return createHmac('sha256', TENANT_ATTESTATION_SECRET)
    .update(tenantId)
    .digest('hex');
}
...
export function tenantForwardHeaders(
  tenantId: string,
): { 'X-Tenant-ID': string; 'X-Tenant-Sig': string } {
  return {
